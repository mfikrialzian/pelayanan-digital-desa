...
# 1. Storage checks
content = re.sub(r"localStorage\.getItem\('isAdminLoggedIn_Narmada'\) === 'true'", "localStorage.getItem('adminToken_Narmada') !== null", content)
...
for ep in endpoints:
    content = re.sub(rf"\.{ep}\(", f".{ep}(localStorage.getItem('adminToken_Narmada'), ", content)
