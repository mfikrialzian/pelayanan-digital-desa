...
old_html_path = 'd:/PelayananDigitalDesa/vercel-frontend/index_old.html'

with open(old_html_path, 'r', encoding='utf-8') as f:
    content = f.read()

start_str = '<div id="subview-admin-layanan" class="hidden w-full max-w-4xl mx-auto mt-4 pb-10">'
end_str = '<div id="subview-admin-daftar-layanan" class="hidden space-y-6 text-left">'
...
old_subview = content[start_idx:end_idx]

step1_start = old_subview.find('<div id="bl-step-1" class="space-y-6">')
footer_start = old_subview.find('<!-- FOOTER NAV BUTTONS -->')

steps_html = old_subview[step1_start:footer_start].strip()
...
<div class="drawer-panel" id="drawer-req">
    ...
    <div class="drawer-body space-y-4">
        <p class="text-xs text-slate-500 italic">Konfigurasi detail persyaratan akan diletakkan di sini pada fase berikutnya.</p>
    </div>
...
<div class="drawer-panel" id="drawer-q">
    ...
    <div class="drawer-body space-y-4">
        <p class="text-xs text-slate-500 italic">Konfigurasi pertanyaan akan dipindahkan ke drawer ini pada fase berikutnya.</p>
    </div>
    <div class="drawer-footer flex justify-end gap-2">
        <button type="button" onclick="closeAllDrawers()" class="cursor-pointer px-4 py-2 bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 rounded-xl text-xs font-bold transition-all shadow-sm">Batal</button>
        <button type="button" onclick="closeAllDrawers()" class="cursor-pointer px-4 py-2 bg-narmadaGreen hover:bg-emerald-700 text-white rounded-xl text-xs font-bold transition-all shadow-sm">Terapkan</button>
...
content = content[:start_idx] + new_subview + content[end_idx:]

with open(html_path, 'w', encoding='utf-8') as f:
    f.write(content)

print("Successfully replaced HTML structure safely.")
