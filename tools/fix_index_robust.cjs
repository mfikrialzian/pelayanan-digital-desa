...
const startMarker = '<h3 id="pengajuan-table-title"';
const endMarker = '<!-- Table Wrapper -->';

let startIndex = html.indexOf(startMarker);
if (startIndex !== -1) {
    // Find the enclosing div
    let divStartIndex = html.lastIndexOf('<div class="p-5 border-b border-slate-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">', startIndex);
    let divEndIndex = html.indexOf(endMarker, startIndex);
    
    if (divStartIndex !== -1 && divEndIndex !== -1) {
        let blockToReplace = html.substring(divStartIndex, divEndIndex);
        ...
        html = html.replace(blockToReplace, newStr);
