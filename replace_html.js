...
const filePath = 'c:\\Users\\alzia\\.gemini\\antigravity\\scratch\\PelayananDigitalDesa\\index.html';
let content = fs.readFileSync(filePath, 'utf-8');

const startIdx = content.indexOf('<div id="subview-admin-layanan" class="hidden space-y-6 text-left">');
const endIdx = content.indexOf('<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start mt-6">');

const newHtml = `                <div id="subview-admin-layanan" class="hidden space-y-6 text-left">
                    <!-- STICKY HEADER -->
...
if (startIdx !== -1 && endIdx !== -1) {
    const newContent = content.substring(0, startIdx) + newHtml + content.substring(endIdx);
    fs.writeFileSync(filePath, newContent, 'utf-8');
    console.log('HTML Replaced successfully with Node');
} else {
    console.log('Error: start or end index not found');
}
