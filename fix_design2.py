...
content = \"\"\"# Panduan Desain Pelayanan Digital Desa Narmada (DESIGN.md)
...
- **Warna Utama (Primary):** \g-narmadaGreen\, \	ext-narmadaGreen\, \order-narmadaGreen\. Warna ini adalah warna hijau emerald khas Desa Narmada.
- **Warna Aksen Hover Utama:** \hover:bg-emerald-600\ atau \hover:bg-emerald-700\.
- **Latar Belakang Dasar (Background):** \g-slate-50\ (untuk halaman/kanvas utama) dan \g-white\ (untuk kartu/elemen spesifik).
- **Warna Teks & Garis (Grayscale):**
  - Judul/Teks Utama: \	ext-slate-800\ atau \	ext-slate-900\.
  - Sub-judul/Deskripsi/Label: \	ext-slate-500\ atau \	ext-slate-400\.
  - Garis Batas (Border): \order-slate-100\ atau \order-slate-200\.

## 3. Tipografi & Ikonografi

- **Sistem Font:** Menggunakan font Sans-serif modern bawaan Tailwind.
- **Ukuran Teks:**
  - \	ext-[10px]\ atau \	ext-xs\ untuk label kecil, *badge*, atau informasi sekunder.
  - \	ext-sm\ untuk teks paragraf atau isian *input*.
  - \	ext-lg\ atau \	ext-xl\ untuk judul utama halaman atau kartu.
- **Ketebalan Teks (Font Weight):**
  - \ont-bold\ atau \ont-semibold\ untuk judul dan elemen interaktif (tombol).
  - \ont-normal\ (default) untuk teks panjang/deskripsi.
- **Ikon:** Menggunakan pustaka **FontAwesome 6** (\a-solid\, \a-regular\). Ikon sering disandingkan dengan margin atau *gap* (\gap-2\, \mr-1.5\, dll).
...
- **Tombol Bahaya (Danger):** \g-red-600 hover:bg-red-700 text-white\.
...
- **Field Non-aktif (Disabled/Read-only):** Tambahkan warna \g-slate-100 text-slate-500 cursor-not-allowed\.
- **Gaya Label:**
...
- *Warna alternatif:* \g-blue-50 text-blue-600\, \g-red-50 text-red-600\, \g-amber-50 text-amber-600\.
...
- **Header (\	head\):** \g-slate-50 text-slate-500 text-[10px] uppercase tracking-wider border-b border-slate-100\ dengan \ont-bold\ di setiap \<th>\.
- **Baris Data (\	r\):** \hover:bg-slate-50 transition-colors border-b border-slate-50 text-xs text-slate-600\.

### F. Modal & *Popup*

- Menggunakan pendekatan *fixed overlay* dengan *backdrop-blur* atau latar belakang semi-transparan.
- **Overlay:** \ixed inset-0 bg-slate-900/50 z-[100] flex items-center justify-center\
- **Konten Modal:** \g-white rounded-2xl shadow-xl border border-slate-100 w-full max-w-sm p-6\

### G. Transisi & Animasi (*Micro-Animations*)

- Seluruh elemen yang bisa berinteraksi wajib diberikan kelas \	ransition-all\ atau \	ransition-colors\.
- Untuk menyorot elemen saat *hover*, gunakan interaksi \group\ dan \group-hover\. Contoh pada menu sidebar: ikon membesar (\group-hover:scale-110\) dan berubah warna ke \
armadaGreen\ saat elemen induknya di-*hover*.

### H. Formulir Panel & Layout Bersih (*Seamless Form Panels*)

- **Kanvas Menyatu (*Seamless Background*):** Bagian utama form (baik *wizard* maupun *form* tunggal) tidak boleh menggunakan warna latar belakang yang kontras (seperti \g-slate-50\) jika berada di bawah *header* berwarna putih. Gunakan warna putih murni (\g-white\) secara menyeluruh untuk bodi panel agar terkesan rata (*flat* dan *clean*).
- **Tanpa Bingkai Bertumpuk (*No Nested Cards*):** Dilarang membungkus input *form* ke dalam kartu tambahan (\g-white p-6 border...\) jika *form* tersebut sudah berada di dalam panel modal atau kontainer utama.
...
3. Jangan pernah melakukan \innerHTML\ tanpa pertimbangan XSS yang ketat, prioritaskan mengubah \	extContent\ atau manipulasi *class*.
